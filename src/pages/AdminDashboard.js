  ...
  if (!user?.isAdmin) {
    return (
      <div className="page-container">
        <h3 className="page-title">Admin access required</h3>
        <p style={{marginTop:'0.5rem'}}>You need to be an admin to view this page.</p>
      </div>
    );
  }
  ...
  return (
    <div className="page-container">
      ...
      <form onSubmit={editingId ? handleUpdate : handleSubmit} className="admin-form">
        ...
        <textarea placeholder="Description" value={form.description} onChange={(e) => setForm({...form, description: e.target.value})} className="admin-input"></textarea>
        <input placeholder="Price" value={form.price} onChange={(e) => setForm({...form, price: e.target.value})} className="admin-input" />
        <input placeholder="Category" value={form.category} onChange={(e) => setForm({...form, category: e.target.value})} className="admin-input" />
        <div className="admin-file-group">
          <label className="admin-file-label">Image</label>
        ...
        <ul className="admin-products-list">
          {products.map(p => (
            <li key={p._id} className="admin-product-item">
              <div>
                <h4 className="admin-product-name">{p.title}</h4>
                <div className="admin-product-meta">${p.price} — {p.category}</div>
              </div>
